...
/**
 * @author liushiming
 * @version EurekaInstanceConfigBean.java, v 0.0.1 2018年5月4日 下午6:34:00 liushiming
 */
@SuppressWarnings("unused")
public class EurekaInstanceConfigBean implements EurekaInstanceConfig, EnvironmentAware {
    ...
    /**
     * Indicates how often (in seconds) the eureka client needs to send heartbeats to eureka server to indicate that it
     * is still alive. If the heartbeats are not received for the period specified in leaseExpirationDurationInSeconds,
     * eureka server will remove the instance from its view, there by disallowing traffic to this instance.
     * <p>
     * Note that the instance could still not take traffic if it implements HealthCheckCallback and then decides to make
     * itself unavailable.
     */
    private int leaseRenewalIntervalInSeconds = 30;

    /**
     * Indicates the time in seconds that the eureka server waits since it received the last heartbeat before it can
     * remove this instance from its view and there by disallowing traffic to this instance.
     * <p>
     * Setting this value too long could mean that the traffic could be routed to the instance even though the instance
     * is not alive. Setting this value too small could mean, the instance may be taken out of traffic because of
     * temporary network glitches.This value to be set to atleast higher than the value specified in
     ...
     * <p>
     * This is typically the way other instance would find this instance by using the virtual host name.Think of this as
     * similar to the fully qualified domain name, that the users of your services will need to find this instance.
     */
     ...
     * <p>
     * This is typically the way other instance would find this instance by using the secure virtual host name.Think of
     * this as similar to the fully qualified domain name, that the users of your services will need to find this
     * instance.
     */
    private String secureVirtualHostName = UNKNOWN;

    /**
     * Gets the AWS autoscaling group name associated with this instance. This information is specifically used in an
     * AWS environment to automatically put an instance out of service after the instance is launched and it has been
     * disabled for traffic..
     */
    private String aSGName;

    /**
     * Gets the metadata name/value pairs associated with this instance. This information is sent to eureka server and
     * can be used by other instances.
     */
    private Map<String, String> metadataMap = new HashMap<>();

    /**
     * Returns the data center this instance is deployed. This information is used to get some AWS specific instance
     * information if the instance is deployed in AWS.
     */
    private DataCenterInfo dataCenterInfo = new MyDataCenterInfo(DataCenterInfo.Name.MyOwn);

    /**
     * Get the IPAdress of the instance. This information is for academic purposes only as the communication from other
     * instances primarily happen using the information supplied in {@link #getHostName(boolean)}.
     */
    private String ipAddress;

    /**
     * Gets the relative status page URL path for this instance. The status page URL is then constructed out of the
     * hostName and the type of communication - secure or unsecure as specified in securePort and nonSecurePort.
     * <p>
    ...
    /**
     * Gets the absolute status page URL path for this instance. The users can provide the statusPageUrlPath if the
     * status page resides in the same instance talking to eureka, else in the cases where the instance is a proxy for
    ...
    /**
     * Gets the relative home page URL Path for this instance. The home page URL is then constructed out of the hostName
     * and the type of communication - secure or unsecure.
     * <p>
    ...
    /**
     * Gets the absolute home page URL for this instance. The users can provide the homePageUrlPath if the home page
     * resides in the same instance talking to eureka, else in the cases where the instance is a proxy for some other
     * server, users can provide the full URL. If the full URL is provided it takes precedence.
     * <p>
     * It is normally used for informational purposes for other services to use it as a landing page. The full URL
     * should follow the format http://${eureka.hostname}:7001/ where the value ${eureka.hostname} is replaced at
     * runtime.
     */
    private String homePageUrl;

    /**
     * Gets the relative health check URL path for this instance. The health check page URL is then constructed out of
     * the hostname and the type of communication - secure or unsecure as specified in securePort and nonSecurePort.
     * <p>
     * It is normally used for making educated decisions based on the health of the instance - for example, it can be
     * used to determine whether to proceed deployments to an entire farm or stop the deployments without causing
     * further damage.
     */
    private String healthCheckUrlPath = "/health";

    /**
     * Gets the absolute health check page URL for this instance. The users can provide the healthCheckUrlPath if the
     ...
     * It is normally used for making educated decisions based on the health of the instance - for example, it can be
     * used to determine whether to proceed deployments to an entire farm or stop the deployments without causing
     * further damage. The full URL should follow the format http://${eureka.hostname}:7001/ where the value
     * ${eureka.hostname} is replaced at runtime.
     */
    private String healthCheckUrl;

    /**
     * Gets the absolute secure health check page URL for this instance. The users can provide the secureHealthCheckUrl
     * if the health check page resides in the same instance talking to eureka, else in the cases where the instance is
     * a proxy for some other server, users can provide the full URL. If the full URL is provided it takes precedence.
     *
     * <p>
     * It is normally used for making educated decisions based on the health of the instance - for example, it can be
     * used to determine whether to proceed deployments to an entire farm or stop the deployments without causing
    ...
    /**
     * Flag to say that, when guessing a hostname, the IP address of the server should be used in prference to the
    ...
    public void setEnvironment(Environment environment) {
        ...
        String springAppName = SpringEnvironmentUtil.getProperty(this.environment, "spring.application." + "name");
    ...
    public int hashCode() {
        return Objects.hash(hostInfo, inetUtils, appname, appGroupName, instanceEnabledOnit, nonSecurePort, securePort,
            nonSecurePortEnabled, securePortEnabled, leaseRenewalIntervalInSeconds, leaseExpirationDurationInSeconds,
            virtualHostName, instanceId, secureVirtualHostName, aSGName, metadataMap, dataCenterInfo, ipAddress,
            statusPageUrlPath, statusPageUrl, homePageUrlPath, homePageUrl, healthCheckUrlPath, healthCheckUrl,
            secureHealthCheckUrl, namespace, hostname, preferIpAddress, initialStatus, defaultAddressResolutionOrder,
            environment);
    }
    ...
    public String toString() {
        return new StringBuilder("EurekaInstanceConfigBean{").append("hostInfo=").append(hostInfo).append(", ")
            ...
            .append(", ").append("leaseRenewalIntervalInSeconds=").append(leaseRenewalIntervalInSeconds).append(", ")
            .append("leaseExpirationDurationInSeconds=").append(leaseExpirationDurationInSeconds).append(", ")
            .append("virtualHostName='").append(virtualHostName).append("', ").append("instanceId='").append(instanceId)
            .append("', ").append("secureVirtualHostName='").append(secureVirtualHostName).append("', ")
            .append("aSGName='").append(aSGName).append("', ").append("metadataMap=").append(metadataMap).append(", ")
            .append("dataCenterInfo=").append(dataCenterInfo).append(", ").append("ipAddress='").append(ipAddress)
            .append("', ").append("statusPageUrlPath='").append(statusPageUrlPath).append("', ")
            .append("statusPageUrl='").append(statusPageUrl).append("', ").append("homePageUrlPath='")
            .append(homePageUrlPath).append("', ").append("homePageUrl='").append(homePageUrl).append("', ")
            .append("healthCheckUrlPath='").append(healthCheckUrlPath).append("', ").append("healthCheckUrl='")
            .append(healthCheckUrl).append("', ").append("secureHealthCheckUrl='").append(secureHealthCheckUrl)
            .append("', ").append("namespace='").append(namespace).append("', ").append("hostname='").append(hostname)
            .append("', ").append("preferIpAddress=").append(preferIpAddress).append(", ").append("initialStatus=")
