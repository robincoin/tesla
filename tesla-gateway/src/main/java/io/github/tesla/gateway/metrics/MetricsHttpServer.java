...
import io.github.tesla.gateway.metrics.handler.HeapDumpHandler;
import io.github.tesla.gateway.metrics.handler.SupportPluginsHandler;
import io.github.tesla.gateway.metrics.handler.ThreadDumpHandler;
...
public class MetricsHttpServer {
    ...
    static class GrayHandler implements HttpHandler {
        @Override
        public void handle(HttpExchange httpExchange) throws IOException {
            ...
            int status = 200;
            try {
                if (requestUrl.getPath().equalsIgnoreCase(MetricsHttpServer.grayApi + "/gray/rules/hash")) {
                    responseString = GrayRuesCache.getHash();
                    httpExchange.getResponseHeaders().set("Content-Type", MediaType.JSON_UTF_8.toString());
                } else if (requestUrl.getPath().equalsIgnoreCase(MetricsHttpServer.grayApi + "/gray/rules")
                    && httpExchange.getRequestMethod().equalsIgnoreCase("get")) {
    ...
    public final static String grayApi = "/grayapi";
    ...
    public MetricsHttpServer(InetSocketAddress addr, CollectorRegistry registry, boolean daemon) throws IOException {
        ...
        server.createContext(grayApi, new GrayHandler());
        server.createContext("/actuator/heapdump", new HeapDumpHandler());
        server.createContext("/actuator/threaddump", new ThreadDumpHandler());
        server.createContext("/supportplugins", new SupportPluginsHandler());
