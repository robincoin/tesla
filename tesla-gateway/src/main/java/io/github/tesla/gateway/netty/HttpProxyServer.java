...
public class HttpProxyServer {
    ...
    public HttpProxyServer(ServerGroup serverGroup, InetSocketAddress requestedAddress,
        ...
        if (writeThrottleBytesPerSecond > 0 || readThrottleBytesPerSecond > 0) {
            this.globalTrafficShapingHandler =
                createGlobalTrafficShapingHandler(readThrottleBytesPerSecond, writeThrottleBytesPerSecond);
        } else {
    ...
    public HttpProxyServerBootstrap clone() {
        return new HttpProxyServerBootstrap(serverGroup,
            ...
            filtersSource, transparent, idleConnectionTimeout, connectTimeout, serverResolver,
            globalTrafficShapingHandler != null ? globalTrafficShapingHandler.getReadLimit() : 0,
            globalTrafficShapingHandler != null ? globalTrafficShapingHandler.getWriteLimit() : 0, localAddress,
            proxyAlias, maxInitialLineLength, maxHeaderSize, maxChunkSize, allowRequestsToOriginServer);
    }
