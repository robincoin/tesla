...
$(function () {
    ...
    serviceCorsConfig.init($('#service_cors_panel'));
    ...
    $("#serviceForm").validate({
        ...
        submitHandler: function (form) {
            swal({
                ...
                pluginList = serviceCorsConfig.getFormatResult($('#service_cors_panel'), pluginList);
...
function addEventOnPluginSelect(endpointDiv) {
    Object.keys(endpointPlugins).forEach(function (key) {
        endpointDiv.find("select[name='endpointPluginsSelect']")
            .append("<option value='" + key + "'>" + endpointPlugins[key] + "</option>");
    });
    endpointDiv.find("select").selectpicker('refresh');
